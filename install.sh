...
cargo build --release --target wasm32-unknown-unknown \
  ...
  -p sort8-bubble -p sort8-network -p sort8-batcher -p sort8-swar -p clz-naive -p clz-branchless -p bitrev-naive -p bitrev-swar \
  -p siphash13-ref -p siphash13-stream -p crc64-bitwise -p crc64-nibble \
  -p morton-naive -p morton-swar -p sbox-scalar -p sbox-table -p sbox-swar
