/* ANV-007 contender: CRC-64/XZ one byte at a time through a 256-entry
 * table - written in C, entered through the anvil's external-lane
 * protocol, no harness rebuild involved.
 *
 * The contract is small: `native --seed S --iters I --repeats R` prints
 * the score JSON, `many --seed S --iters I` writes one little-endian u64
 * output per input word. The input stream is the anvil's pinned xorshift
 * generator. Admission is exactly like a Rust lane: the Lean model
 * `Razor.Anvil.crcByte` carries these 256 constants and the proof checks
 * the walk against the bit-at-a-time spec on all 2^64 inputs.
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static uint64_t solve(uint64_t x) {
    uint64_t crc = ~x;
    for (int i = 0; i < 8; i++)
...
static uint64_t next_input(uint64_t *state) {
    *state ^= *state << 13;
    *state ^= *state >> 7;
    *state ^= *state << 17;
    return *state; /* this challenge's input map is the identity */
}

static uint64_t run_stream(uint64_t seed, uint64_t iters) {
    uint64_t state = seed | 1, acc = 0;
    for (uint64_t i = 0; i < iters; i++)
        acc += solve(next_input(&state));
    return acc;
}
...
static uint64_t flag(int argc, char **argv, const char *name, uint64_t dflt) {
...
int main(int argc, char **argv) {
    ...
    uint64_t seed = flag(argc, argv, "--seed", 0xC0FFEE);
    uint64_t iters = flag(argc, argv, "--iters", 10000);

    if (strcmp(cmd, "many") == 0) {
        uint64_t state = seed | 1;
        for (uint64_t i = 0; i < iters; i++) {
            uint64_t y = solve(next_input(&state));
            fwrite(&y, 8, 1, stdout); /* little-endian on every anvil rig */
        }
        return 0;
    }
    if (strcmp(cmd, "native") == 0) {
        uint64_t repeats = flag(argc, argv, "--repeats", 9);
        if (repeats > 99) repeats = 99;
        uint64_t checksum = run_stream(seed, iters); /* warm-up */
        uint64_t times[99];
        for (uint64_t r = 0; r < repeats; r++) {
            /* run_stream is pure and its arguments never change, so an
             * optimizer may hoist the call out of this loop and leave the
             * clock timing nothing (gcc -O3 does). The empty asm makes the
             * seed opaque per repeat and the result observed. */
            uint64_t s = seed;
            __asm__ volatile("" : "+r"(s));
            uint64_t t0 = now_ns();
            uint64_t c = run_stream(s, iters);
            __asm__ volatile("" : : "r"(c) : "memory");
            times[r] = now_ns() - t0;
            if (c != checksum) { fprintf(stderr, "checksum drift\n"); return 2; }
        }
        qsort(times, repeats, 8, cmp_u64);
        uint64_t median = times[repeats / 2];
...
#endif
        printf("{\"tier\":\"native\",\"arch\":\"%s\",\"impl\":\"crc64-byte-c\","
               "\"seed\":%llu,\"iters\":%llu,\"ns\":%llu,\"ns_per_op\":%.2f,\"checksum\":%llu}\n",
               arch, (unsigned long long)seed, (unsigned long long)iters,
               (unsigned long long)median, (double)median / (double)iters,
               (unsigned long long)checksum);
        return 0;
    }
    fprintf(stderr, "usage: crc64-byte-c <native|many> [--seed S] [--iters I] [--repeats R]\n");
    return 2;
}
