...
registry's answer is `razor repin`: a hole moves to a new wording only if
a proof that the two wordings are equivalent kernel-checks. This file
holds that equivalence for hole RZR-104, whose original wording binds its
...
the hole; the old wording, the new wording, and this proof all stay on
the log. -/
