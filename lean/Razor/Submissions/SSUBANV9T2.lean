...
from-the-definition S-box, checked by kernel evaluation over the whole
byte domain (`decide` - no axioms beyond `propext`, so it survives the
registry's axiom allowlist, unlike `native_decide`). Stated over
`BitVec 8` so the quantifier is finite. -/
